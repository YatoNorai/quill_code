...
val buildRustTasks = rustAbis.map { abi ->
    tasks.register("buildRust_${abi.androidAbi}") {
        ...
        doLast {
            ...
            // On fresh CI machines (or first-time dev setups) the Android targets
            // are not installed by default.  `rustup target add` is idempotent —
            // it is a no-op when the target is already present.
            val rustup = File(cargo).resolveSibling("rustup").let {
                if (it.exists()) it.absolutePath else "rustup"
            }
            logger.lifecycle("→ rustup target add ${abi.rustTarget}")
            exec {
                commandLine(rustup, "target", "add", abi.rustTarget)
                val cargoParent = File(cargo).parent
                if (cargoParent != null) {
                    val currentPath = System.getenv("PATH") ?: ""
                    environment("PATH", "$cargoParent${File.pathSeparator}$currentPath")
                }
            }
